        ...
        if (bytesRead > 0)
        {
            ...
            if (found1 != nullptr || found2 != nullptr)
            {
                ...
                for (i = 0; i < arrReceiveData.size(); i++)
                {
                    ...
                    {
                        status = arrReceiveData[i];
                        std::cout << "Status : " << status << std::endl;
                    }
                    else if (arrReceiveData[i].find("MPos:") != std::string::npos)
                    {
                        mpos = splitText<float>(arrReceiveData[i].substr(5), ',');
                        std::cout << "MPos : ";
                        for (int j = 0; j < mpos.size(); j++) 
                        {
                            j == 0 ? std::cout << mpos[j] : std::cout << "," << mpos[j];
                        }
                        std::cout << std::endl;
                    }
                    else if (arrReceiveData[i].find("FS:") != std::string::npos)
                    {
                        fs = splitText<float>(arrReceiveData[i].substr(3), ',');
                        std::cout << "FS : ";
                        for (int j = 0; j < fs.size(); j++)
                        {
                            j == 0 ? std::cout << fs[j] : std::cout << "," << fs[j];
                        }
                        std::cout << std::endl;
                    }
                    else if (arrReceiveData[i].find("Pn:") != std::string::npos)
                    {
                        ...
                        {
                            j == 0 ? std::cout << pn[j] : std::cout << "," << pn[j];
                        }
                        std::cout << std::endl;
                    }
                    else if (arrReceiveData[i].find("WCO:") != std::string::npos)
                    {
                        wco = splitText<float>(arrReceiveData[i].substr(4), ',');
                        std::cout << "WCO : ";
                        for (int j = 0; j < wco.size(); j++)
                        {
                            j == 0 ? std::cout << wco[j] : std::cout << "," << wco[j];
                        }
                        std::cout << std::endl;
                    }
                    else if (arrReceiveData[i].find("Ov:") != std::string::npos)
                    {
                        ov = splitText<float>(arrReceiveData[i].substr(3), ',');
                        std::cout << "Ov : ";
                        for (int j = 0; j < ov.size(); j++)
                        {
                            j == 0 ? std::cout << ov[j] : std::cout << "," << ov[j];
                        }
                        std::cout << std::endl;
                    }
                    else if (arrReceiveData[i].find("os:") != std::string::npos)
                    {
                        ...
                        {
                            j == 0 ? std::cout << os[j] : std::cout << "," << os[j];
                        }
                        std::cout << std::endl;
                    }
                    else
